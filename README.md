...
3. **Step 1** — Review detected rooms. **Exists** rooms are reused (never duplicated, never renamed). **New** rooms can be created. Edit aliases freely; new room names are editable. Create never posts a second room whose name matches an existing one (same check as matching: case, punctuation, and apostrophes are ignored).
